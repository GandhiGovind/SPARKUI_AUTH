...
public class LDAPUserAuthentication {
	...
	public String authenticateUser(String username, String password) {
		...
		String name = "";
		System.setProperty("javax.net.ssl.trustStore", SPARKUIConstants.LDAP_CERTIFICATION_PATH);
		...
		while (count < 5 && !responseOk) {
			...
			env.put(Context.PROVIDER_URL, SPARKUIConstants.PROVIDER_URL);
			// env.put(Context.SECURITY_AUTHENTICATION, "simple");
			// env.put(Context.SECURITY_PRINCIPAL, "uid=<userid>");
			env.put(Context.SECURITY_PRINCIPAL, "cn=" + username + ",ou=users,o=<OrgName>");
			...
			try {
				...
				System.out.println("name is ldap:" + name + "-" + String.valueOf(name.length()));
				if (name.isEmpty()) {
					name = "name:" + username;
				}
				ctx.close();
	...
	private String getUserName(String userID, LdapContext ctx, SearchControls searchControls) {
